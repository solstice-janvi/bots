        ...
        try {
            switch (userData.sessionState) {
                ...
                case USER_STATES.MAIN_MENU:
                    if (incomingMsg === 'quiz') { // Now directly triggers AI quiz flow
                    ...
                    } else if (incomingMsg === 'change exam' || incomingMsg === 'change subject' || incomingMsg === 'restart') {
                        ...
                        responseMessage = `I didn't understand that. Please type *quiz*, *flashcards*, *change exam*, *change subject*, or *restart*.`; // Updated prompt
                    }
                    break;

                case USER_STATES.AWAITING_AI_QUIZ_TOPIC: // This state is now directly for "quiz"
                ...
                case USER_STATES.AWAITING_QUIZ_ANSWER:
                    ...
                    if (userData.currentQuiz.currentQIndex < userData.currentQuiz.questionIds.length) {
                        ...
                        // Quiz finished
                        responseMessage += `\n\n🎉 Quiz finished! You scored ${userData.currentQuiz.score} out of ${userData.currentQuiz.questionIds.length}.\n\nWhat next? Type *quiz* for another, *flashcards* for revision, *change exam* / *change subject*, or *restart*.`; // Updated prompt
                        userData.sessionState = USER_STATES.MAIN_MENU;
                ...
                case USER_STATES.AWAITING_FLASHCARD_TOPIC:
                    ...
                    responseMessage = `Here's your ${contentType} for "${topic}":\n\n${revisionContent}\n\nWhat next? Type *quiz* for a quiz, *flashcards* for more revision, *change exam* / *change subject*, or *restart*.`; // Updated prompt
                    userData.sessionState = USER_STATES.MAIN_MENU;
                    break;
